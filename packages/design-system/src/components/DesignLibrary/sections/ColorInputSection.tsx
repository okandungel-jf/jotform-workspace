...
export interface ColorInputPanelState {
  size: 'sm' | 'md' | 'lg';
...
export const defaultColorInputState: ColorInputPanelState = {
  size: 'md',
  status: 'default',
  disabled: false,
  darkPreview: false,
...
export function ColorInputSection({ state }: { state: ColorInputPanelState }) {
  ...
  return (
    <div>
      <h1 className="dl-section__title">Color Input</h1>
      <p className="dl-section__description">
        Color input with hex value text field and color swatch preview. Click the swatch to open the native color picker.
      </p>
      ...
      <div className="dl-stories">
        ...
        <div className="dl-stories__list">
          <div>
            <h3 className="dl-stories__item-title">Default Color Input</h3>
            <div className="dl-stories__item-preview">
              <div style={{ width: 340 }}><ColorInput size={state.size} /></div>
            </div>
          </div>
          <div>
            <h3 className="dl-stories__item-title">Error Color Input</h3>
            <div className="dl-stories__item-preview">
              <div style={{ width: 340 }}><ColorInput size={state.size} status="error" /></div>
            </div>
          </div>
          <div>
            <h3 className="dl-stories__item-title">Success Color Input</h3>
            <div className="dl-stories__item-preview">
              <div style={{ width: 340 }}><ColorInput size={state.size} status="success" /></div>
            </div>
          </div>
          <div>
            <h3 className="dl-stories__item-title">Read Only Color Input</h3>
            <div className="dl-stories__item-preview">
              <div style={{ width: 340 }}><ColorInput size={state.size} status="readonly" /></div>
            </div>
          </div>
          <div>
            <h3 className="dl-stories__item-title">Disabled Color Input</h3>
            <div className="dl-stories__item-preview">
              <div style={{ width: 340 }}><ColorInput size={state.size} disabled /></div>
            </div>
          </div>
        </div>
      </div>
...
export function ColorInputPanel({
  ...
  state: ColorInputPanelState;
  onChange: (state: ColorInputPanelState) => void;
}) {
  const update = (partial: Partial<ColorInputPanelState>) => onChange({ ...state, ...partial });
    ...
    <>
      ...
      <div className="dl-playground__field">
        ...
        <select
          ...
          value={state.status}
          onChange={(e) => update({ status: e.target.value as ColorInputPanelState['status'] })}
        >
