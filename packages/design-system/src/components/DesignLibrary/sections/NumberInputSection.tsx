...
export interface NumberInputPanelState {
  size: 'sm' | 'md' | 'lg';
...
export const defaultNumberInputState: NumberInputPanelState = {
  size: 'md',
  status: 'default',
  showUnit: true,
  unit: 'kg',
...
export function NumberInputSection({ state }: { state: NumberInputPanelState }) {
  ...
  return (
    <div>
      <h1 className="dl-section__title">Number Input</h1>
      <p className="dl-section__description">
        Number input with increment/decrement arrows and optional unit suffix.
      </p>
      ...
      <div className="dl-stories">
        ...
        <div className="dl-stories__list">
          <div>
            <h3 className="dl-stories__item-title">Default Number Input</h3>
            <div className="dl-stories__item-preview">
              <div style={{ width: 340 }}><NumberInput size={state.size} placeholder="0" /></div>
            </div>
          </div>
          <div>
            <h3 className="dl-stories__item-title">Error Number Input</h3>
            <div className="dl-stories__item-preview">
              <div style={{ width: 340 }}><NumberInput size={state.size} status="error" placeholder="0" /></div>
            </div>
          </div>
          <div>
            <h3 className="dl-stories__item-title">Success Number Input</h3>
            <div className="dl-stories__item-preview">
              <div style={{ width: 340 }}><NumberInput size={state.size} status="success" placeholder="0" /></div>
            </div>
          </div>
          <div>
            <h3 className="dl-stories__item-title">Read Only Number Input</h3>
            <div className="dl-stories__item-preview">
              <div style={{ width: 340 }}><NumberInput size={state.size} status="readonly" placeholder="0" /></div>
            </div>
          </div>
          <div>
            <h3 className="dl-stories__item-title">Disabled Number Input</h3>
            <div className="dl-stories__item-preview">
              <div style={{ width: 340 }}><NumberInput size={state.size} disabled placeholder="0" /></div>
            </div>
          </div>
        </div>
      </div>
...
export function NumberInputPanel({
  ...
  state: NumberInputPanelState;
  onChange: (state: NumberInputPanelState) => void;
}) {
  const update = (partial: Partial<NumberInputPanelState>) => onChange({ ...state, ...partial });
    ...
    <>
      ...
      <div className="dl-playground__field">
        ...
        <select
          ...
          value={state.status}
          onChange={(e) => update({ status: e.target.value as NumberInputPanelState['status'] })}
        >
      ...
      <FormField size="md" title="Unit" showTitle showDescription={false} showHelpText={false}>
        <Input size="sm" value={state.unit} onChange={(e) => update({ unit: e.target.value })} />
      </FormField>

      <div className="dl-playground__divider" />

      <ToggleField label="Show Unit" value={state.showUnit} onChange={(v) => update({ showUnit: v })} />
